  ...
  return (
    <>
      <IonCard className="speaker-card">
        <IonCardHeader>
          <IonItem button detail={false} lines="none" className="speaker-item" href={`/tabs/speakers/${speaker.id}`}>
        ...
        <IonCardContent>
          <IonList lines="none">
            {sessions.map(session => (
              <IonItem detail={false} href={`/tabs/speakers/sessions/${session.id}`} key={session.name}>
                <IonLabel>
                  <h3>{session.name}</h3>
                </IonLabel>
              </IonItem>
            ))}
            <IonItem detail={false} href={`/tabs/speakers/${speaker.id}`}>
              <IonLabel>
